...
export async function deliverOrder({ reference, customerEmail, metadata, siteUrl }) {
  ...
  const sibtechOrderItems = orderItems.filter((i) => i.format === "sibtech");
  const webinarOrderItems = orderItems.filter((i) => i.format === "webinar");
  const ebookOrderItems = orderItems.filter(
    (i) => i.format !== "paperback" && i.format !== "resource" && i.format !== "sibtech" && i.format !== "webinar"
  );
  ...
  let sibtechWithLinks = [];
  if (sibtechOrderItems.length > 0) {
    const sibtechSlugs = sibtechOrderItems.map((i) => i.slug).filter(Boolean);
    const sibtechProducts = await client.fetch(
      `*[_type == "sibtechProduct" && slug.current in $slugs]{ title, "slug": slug.current, digitalFile{ asset-> { _id, url } } }`,
      { slugs: sibtechSlugs }
    );
    const deliverable = sibtechProducts.filter((p) => p.digitalFile?.asset?._id);
    sibtechWithLinks = deliverable.map((p) => {
      const token = signDownloadToken({
        assetId: p.digitalFile.asset._id,
        title: p.title,
        ref: reference,
      });
      return {
  ...
  allDownloadLinks.push(...sibtechWithLinks);
  ...
  if (paperbackOrderItems.length > 0) {
    ...
    await client.create({
      ...
      reference,
      email: customerEmail,
     items: paperbackOrderItems.map((i) => ({ _key: crypto.randomUUID(), title: i.title, qty: i.qty })),
  ...
  await sendOrderEmail(customerEmail, {
    ...
    const itemsDelivered = [
       ...booksWithLinks.map((b) => ({ _key: crypto.randomUUID(), title: b.title, slug: b.slug, type: "ebook" })),
    ...resourcesWithLinks.map((r) => ({ _key: crypto.randomUUID(), title: r.title, slug: r.slug, type: "resource" })),
    ...sibtechWithLinks.map((p) => ({ _key: crypto.randomUUID(), title: p.title, slug: p.slug, type: "sibtech" })),
  ];
  await client.create({
    _type: "delivery",
    reference,
