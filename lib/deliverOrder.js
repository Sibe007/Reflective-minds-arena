 ...
 * Ebook items get signed download links emailed. Paperback items create a
 * Physical Order document in Sanity (status "Pending") for fulfillment tracking,
 * and get a "your order is confirmed, we'll ship soon" section in the same email.
...
export async function deliverOrder({ reference, customerEmail, metadata, siteUrl }) {
  ...
  const ebookOrderItems = orderItems.filter((i) => i.format !== "paperback");
  const paperbackOrderItems = orderItems.filter((i) => i.format === "paperback");

  if (ebookOrderItems.length === 0 && paperbackOrderItems.length === 0) {
  ...
  return { delivered: true, books: booksWithLinks, physicalOrderCreated: paperbackOrderItems.length > 0 };
}